...
describe('Slidable component', () => {
    let createElement;
    let createResizableElement;
    let createElementVisibleKey;
    let scope;
    let element;
    ...
    beforeEach(inject(($rootScope, $compile) => {
        ...
        createElement = (side) => {
            const template = `
                <sc-slidable visible="visible" side="${side}"
        ...
        createElementVisibleKey = (side) => {
            const template = `
                <sc-slidable visible="visible" side="${side}"
                    control-bar="controlBar"
                    visible-state-key="${visibleStateKey}">
                    content
                </sc-slidable>
            `;
            element = $compile(template)(scope);
    ...
    it("should show '>' when left slidable is hidden", () => {
        ...
        createElement('left');
        // then
        const actionOnlySpan = element.find('.action')
            .eq(0)
            .find('span')
            .eq(0);
        const displayedActionText = actionOnlySpan.find('span')
            .not('.ng-hide')
            .eq(0)
            .text();
    ...
    it('should set flex constant size on slidable creation if there is a size in localStorage',
        inject(($window) => {
            // given
    ...
    describe('event listeners', () => {
        describe('drag flag', () => {
            ...
            it('should set drag flag to false on window mouseup and save size in storage',
                inject(($window) => {
                    // given
                    scope.visible = true;

                    createResizableElement('left');
                    const eventMove = new angular.element.Event('mousemove');
                    eventMove.clientX = 700;
                    const eventUp = new angular.element.Event('mouseup');
                    const ctrl = element.controller('scSlidable');
                    ctrl.drag = true;
            ...
            it('should set drag flag to false on window mouseup and set left panel width',
                inject(($window) => {
                    // given
                    scope.visible = true;
                    ...
                    // then
                    expect(element.css('flex')).toBe('0 0 400px');
                }
            ));

            it('should set drag flag to false on window mouseup and set right panel width',
                inject(($window) => {
                    // given
                    scope.visible = true;
                    ...
                    // then
                    expect(element.css('flex')).toBe(`0 0 ${width}px`);
                }
            ));
